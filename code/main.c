...
void InitDisplay() {
  ...
  XIo_Out32(XPAR_DVMA_0_BASEADDR + blDvmaFBAR,
            XPAR_DDR2_SDRAM_MPMC_BASEADDR);                 // frame base addr
  XIo_Out32(XPAR_DVMA_0_BASEADDR + blDvmaFLSR, 0x00000A00); // frame line stride
  XIo_Out32(XPAR_DVMA_0_BASEADDR + blDvmaCR,
            0x00000003); // dvma enable, dfl enable
}
...
void MAP_UpdatePath(Pos *pos, int16_t velocity, int16_t radius) {
  ...
  if (n_x < 0.0 || n_x >= 1.0 * MAP_L_X || n_y < 0.0 ||
      n_y >= 1.0 * MAP_L_Y) // out-of-bound check
    return;
...
int GYRO_InitSpi(XSpi *InstancePtr) {
  ...
  Status = XSpi_SetOptions(
      InstancePtr, (XSP_MASTER_OPTION | XSP_CLK_ACTIVE_LOW_OPTION |
...
u8 GYRO_ReadRegByte(XSpi *InstancePtr, u8 reg) {
  ...
  // if (rv == XST_FAILURE) xil_printf("ERROR: spi transfer when reading from
  // reg failed\r\n"); xil_printf("reg out: buf[0] = 0x%02x, buf[1] =
...
void GYRO_GetBias(XSpi *InstancePtr, int16_t *bias_roll, int16_t *bias_pitch,
                  int16_t *bias_yaw) {
...
int main() {
  ...
  // GPIO
  XGpio sw_gpio;
  if (XGpio_Initialize(&sw_gpio, XPAR_FPGA_0_DIP_SWITCHES_8BIT_DEVICE_ID) !=
      XST_SUCCESS) {
  ...
  XGpio led_gpio;
  if (XGpio_Initialize(&led_gpio, XPAR_FPGA_0_LEDS_8BIT_DEVICE_ID) !=
      XST_SUCCESS) {
  ...
  while (XGpio_DiscreteRead(&sw_gpio, SWITCH_CHANNEL) & SWITCH_ON) {
  } /*spin until ON switch flipped */

  while (!(XGpio_DiscreteRead(&sw_gpio, SWITCH_CHANNEL) &
           SWITCH_ON)) { /* looping main routine while ON switch is flipped */
    ...
    if (!(XGpio_DiscreteRead(&sw_gpio, SWITCH_CHANNEL) &
          SWITCH_RUNNING)) { /* if RUNNING switches are flipped, intialize */
      XGpio_DiscreteWrite(
          &led_gpio, LED_CHANNEL,
          LED_INITIALIZING); /* flag leds, tells user to hold the FPGA level */
      ...
      while (!(XGpio_DiscreteRead(&sw_gpio, SWITCH_CHANNEL) &
               SWITCH_RUNNING)) { /* reading & transmitting while RUNNING
